...
inputs = {
  access_token         = local.vars.AMPLIFY_BO_GIT_TOKEN
  name                 = local.name
  description          = local.name
  repository           = local.vars.AMPLIFY_BO_GIT_SOURCE
  ...
  basic_auth_credentials      = local.vars.AMPLIFY_BO_CREDENTIALS
  ...
  environment_variables = local.vars.AMPLIFY_BO_VARIABLES

  environments = {
    main = {
      enable_notification         = true
      branch_name                 = "${local.vars.AMPLIFY_BO_BRANCH}"
  ...
  domain_config = {
    domain_name            = local.vars.AMPLIFY_BO_DNS
