...
locals {
  ...
  name         = "aurora-serverless-${local.vars.ENV}-mysql8-flexibility"
  service_vars = read_terragrunt_config(find_in_parent_folders("service.hcl"))
  tags         = merge(local.service_vars.locals.tags, { name = local.name })
}

dependencies {
  paths = ["../../vpc/network-1", "../../../../network/us-east-1/vpc/network-1/", "../../../../shared/us-east-1/vpc/network-1/"]
}
...
inputs = {
  ...
  security_group_rules = {
    ...
    vpc_shared_ingress = {
      cidr_blocks = dependency.vpc_shared.outputs.private_subnets_cidr_blocks
    }
  ...
  master_username = local.vars.MYSQL_FLEXIBILITY_USERNAME
  master_password = local.vars.MYSQL_FLEXIBILITY_PASSWORD
