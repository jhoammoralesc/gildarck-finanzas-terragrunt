...
locals {
  ...
  name         = "eks-ic-external-secrets-operator-role"
...
dependencies {
  paths = [
    "../eks-ic-external-secrets-operator-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-external-secrets-operator-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:external-secrets:*"]
