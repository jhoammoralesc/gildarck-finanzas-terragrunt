...
locals {
  ...
  name         = "eks-flexy-payway-connector-role"
...
dependencies {
  paths = [
    "../eks-flexy-payway-connector-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-payway-connector-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-payway-connector"]
