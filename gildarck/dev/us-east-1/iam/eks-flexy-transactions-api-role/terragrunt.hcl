...
locals {
  ...
  name         = "eks-flexy-transactions-api-role"
...
dependencies {
  paths = [
    "../eks-flexy-transactions-api-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-transactions-api-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-transactions-api"]
