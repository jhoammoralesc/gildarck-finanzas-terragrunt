...
locals {
  ...
  name         = "eks-ic-api-agreement-manager-role"
...
dependencies {
  paths = [
    "../eks-ic-api-agreement-manager-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-api-agreement-manager-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-api-agreement-manager"]
