...
locals {
  ...
  name         = "eks-ic-acl-santander-role"
...
dependencies {
  paths = [
    "../eks-ic-acl-santander-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-acl-santander-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-acl-santander"]
