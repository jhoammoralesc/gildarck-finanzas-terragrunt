...
locals {
  ...
  name         = "eks-flexy-auth-server-role"
...
dependencies {
  paths = [
    "../eks-flexy-auth-server-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-auth-server-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-auth-server"]
