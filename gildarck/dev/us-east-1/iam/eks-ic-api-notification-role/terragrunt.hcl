...
locals {
  ...
  name         = "eks-ic-api-notification-role"
...
dependencies {
  paths = [
    "../eks-ic-api-notification-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-api-notification-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-api-notification"]
