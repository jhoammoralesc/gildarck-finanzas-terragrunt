...
dependencies {
  paths = [
    "../eks-ebs-csi-controller-role-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "eks-ebs-csi-controller-role-policy" {
  config_path = "../eks-ebs-csi-controller-role-policy"
}
...
inputs = {
  ...
  role_policy_arns = [
    dependency.eks-ebs-csi-controller-role-policy.outputs.arn
  ]
