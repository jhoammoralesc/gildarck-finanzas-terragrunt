...
locals {
  ...
  name         = "eks-ic-api-payment-processor-role"
...
dependencies {
  paths = [
    "../eks-ic-api-payment-processor-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-api-payment-processor-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-api-payment-processor"]
