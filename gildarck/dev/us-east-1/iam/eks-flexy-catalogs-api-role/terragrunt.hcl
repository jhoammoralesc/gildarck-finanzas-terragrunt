...
locals {
  ...
  name         = "eks-flexy-catalogs-api-role"
...
dependencies {
  paths = [
    "../eks-flexy-catalogs-api-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-catalogs-api-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-catalogs-api"]
