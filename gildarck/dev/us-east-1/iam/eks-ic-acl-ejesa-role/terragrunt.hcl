...
dependencies {
  paths = [
    "../eks-ic-acl-ejesa-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-acl-ejesa-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-acl-ejesa"]
