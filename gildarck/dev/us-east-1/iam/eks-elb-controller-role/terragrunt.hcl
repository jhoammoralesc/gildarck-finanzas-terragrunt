...
dependencies {
  paths = [
    "../eks-elb-controller-role-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "eks-elb-controller-role-policy" {
  config_path = "../eks-elb-controller-role-policy"
}
...
inputs = {
  ...
  role_policy_arns = [
    dependency.eks-elb-controller-role-policy.outputs.arn
  ]
