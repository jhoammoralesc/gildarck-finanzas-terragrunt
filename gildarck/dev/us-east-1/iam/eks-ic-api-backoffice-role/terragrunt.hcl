...
locals {
  ...
  name         = "eks-ic-api-backoffice-role"
...
dependencies {
  paths = [
    "../eks-ic-api-backoffice-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-api-backoffice-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-api-backoffice"]
