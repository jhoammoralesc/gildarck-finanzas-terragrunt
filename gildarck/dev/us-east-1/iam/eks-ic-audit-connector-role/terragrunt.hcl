...
locals {
  ...
  name         = "eks-ic-audit-connector-role"
...
dependencies {
  paths = [
    "../eks-ic-audit-connector-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-audit-connector-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-audit-connector"]
