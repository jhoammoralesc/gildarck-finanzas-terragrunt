...
locals {
  ...
  name         = "eks-flexy-credentials-api-role"
...
dependencies {
  paths = [
    "../eks-flexy-credentials-api-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-credentials-api-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-credentials-api"]
