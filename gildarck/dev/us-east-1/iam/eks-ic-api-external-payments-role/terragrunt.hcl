...
locals {
  ...
  name         = "eks-ic-api-external-payments-role"
...
dependencies {
  paths = [
    "../eks-ic-api-external-payments-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-ic-api-external-payments-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:gildarck-${local.vars.ENV}:ic-api-external-payments"]
