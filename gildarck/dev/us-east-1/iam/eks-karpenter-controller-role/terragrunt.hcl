...
dependencies {
  paths = [
    "../eks-karpenter-controller-role-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "eks-karpenter-controller-role-policy" {
  config_path = "../eks-karpenter-controller-role-policy"
}
...
inputs = {
  ...
  role_policy_arns = [
    dependency.eks-karpenter-controller-role-policy.outputs.arn
  ]
