...
locals {
  ...
  name         = "eks-flexy-payment-manager-role"
...
dependencies {
  paths = [
    "../eks-flexy-payment-manager-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-payment-manager-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-payment-manager"]
