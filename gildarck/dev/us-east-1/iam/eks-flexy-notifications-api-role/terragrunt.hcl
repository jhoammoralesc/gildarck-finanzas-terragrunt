...
locals {
  ...
  name         = "eks-flexy-notifications-api-role"
...
dependencies {
  paths = [
    "../eks-flexy-notifications-api-policy",
    "../../eks/eks-${local.vars.ENV}-1/"
  ]
}

dependency "policy" {
  config_path = "../eks-flexy-notifications-api-policy"
}
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:flexibility-${local.vars.ENV}:flexi-notifications-api"]
