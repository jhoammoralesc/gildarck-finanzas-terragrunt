...
terraform {
  ...
  before_hook "build_lambda" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "cd lambda && npm install && zip -r ../ic-flexi-lambda-authorizer.zip index.js node_modules && cd .."]
  }
}
...
locals {
  ...
  name           = "ic-flexi-lambda-authorizer"
...
inputs = {
  ...
  local_existing_package            = "ic-flexi-lambda-authorizer.zip"
  memory_size                       = 1024
