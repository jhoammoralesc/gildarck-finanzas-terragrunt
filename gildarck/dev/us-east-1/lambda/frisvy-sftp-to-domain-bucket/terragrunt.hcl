...
terraform {
  ...
  before_hook "create_lambda_zip" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "zip -j gildarck-sftp-to-domain-bucket.zip lambda/*"]
  }
}
...
locals {
  ...
  name           = "gildarck-sftp-to-domain-bucket"
...
inputs = {
  ...
  local_existing_package            = "gildarck-sftp-to-domain-bucket.zip"
  memory_size                       = 1024
