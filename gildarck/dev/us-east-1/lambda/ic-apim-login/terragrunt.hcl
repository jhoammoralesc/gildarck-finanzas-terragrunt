...
terraform {
  ...
  before_hook "create_lambda_zip" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "zip -j ic-apim-login.zip lambda/*"]
  }
}
...
inputs = {
  ...
  description                       = "Lambda function with authentication logic for APIM."
  ...
  local_existing_package            = "ic-apim-login.zip"
  memory_size                       = 1024

  environment_variables = {
    USER_POOL_ID = "us-east-1_SqRB9UaHj"
    CLIENT_ID    = "454qelaoerh3efnhi356a3n04f"
  }
  ...
  policy_statements = {
    ParameterStore = {
      effect    = "Allow",
      actions   = ["cognito-idp:AdminUpdateUserAttributes", "cognito-idp:InitiateAuth"],
      resources = ["*"]
    }
  }
