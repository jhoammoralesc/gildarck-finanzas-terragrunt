...
terraform {
  ...
  before_hook "build_lambda" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "cd lambda && zip -r ../function-authorizer-maintenance.zip index.js && cd .."]
  }
}
...
locals {
  ...
  name           = "function-authorizer-maintenance"
...
inputs = {
  ...
  description                       = "Lambda function to use as authorizer under maintenance hours."
  ...
  local_existing_package            = "function-authorizer-maintenance.zip"
  memory_size                       = 1024
