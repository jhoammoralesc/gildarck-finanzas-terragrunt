...
terraform {
  ...
  before_hook "create_lambda_zip" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "zip -j ic-apim-list-services.zip lambda/*"]
  }
}
...
inputs = {
  ...
  description                       = "Lambda function to list services APIM."
  ...
  local_existing_package            = "ic-apim-list-services.zip"
  memory_size                       = 1024
