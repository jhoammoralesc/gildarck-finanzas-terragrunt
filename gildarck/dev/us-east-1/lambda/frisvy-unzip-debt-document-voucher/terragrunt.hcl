...
terraform {
  ...
  before_hook "create_lambda_zip" {
    commands = ["apply", "plan"]
    execute  = ["bash", "-c", "zip -j gildarck-unzip-debt-document-voucher.zip lambda/*"]
  }
}
...
locals {
  ...
  name           = "gildarck-unzip-debt-document-voucher"
...
inputs = {
  ...
  local_existing_package            = "gildarck-unzip-debt-document-voucher.zip"
  memory_size                       = 2048
  ...
  policy_statements = {
    S3Access = {
      ...
      resources = [
        "arn:aws:s3:::ic-dev-file-document-publication",
        "arn:aws:s3:::ic-dev-file-document-publication/*"
      ]
    }
  }
